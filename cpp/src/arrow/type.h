...
/*
Field 是一个重要的构建块。它代表一列数据，并包含了该列的名称和类型以及其他元信息
Field 由以下主要组件构成：
Name：字段（即列）的名称。
Data Type：字段值的数据类型。例如，INT32、FLOAT、STRING 等。
Nullable：一个布尔值，指示此字段的值是否可以为 null。
Metadata：一个可选的键值元数据，用于存储关于此字段的额外信息。

Field 对象通常与 Schema 对象一起使用，后者表示一种表格数据结构，其本质上是一个有序的
Field 对象集合。
*/
...
class ARROW_EXPORT DurationType : public TemporalType, public ParametricType {
 public:
  ...
  /* 秒（SECOND）、毫秒（MILLI）、微秒（MICRO）和纳秒（NANO）
   * 下面的代码创建了一个表示毫秒的 DurationType：
   *  std::shared_ptr<DataType> type = duration(TimeUnit::MILLI);
   */
